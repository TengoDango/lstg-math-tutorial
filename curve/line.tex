...
匀速运动是非常简单的一类运动, 我们大家已经非常了解了, 我也没什么好讲的.
我们主要探讨一下直线的各种方程.
...
% 我们来品鉴两个在网上找到的多边形方程, 看一下为什么不推荐这些方程.

% \begin{enumerate}
%   \item 极坐标方程
%   $$
%   r(\theta) = \dfrac{R\cos(180\degree / n)}
%   {\cos((\theta - 180\degree / n) \bmod (360\degree / n)
%   - 180\degree / n)}
%   $$
%   这个方程看起来比较复杂, 但实质上就是直线的极坐标方程的改写.
%   前面没有介绍直线的极坐标方程, 不过我们可以看一个比较简单的:
%   水平线的极坐标方程$r\sin\theta=y_0$ (基于$y=y_0,y=r\sin\theta$).
%   假设我们要用这个极坐标方程制作弹幕, 那么一般让极角等距分布.
%   如下图, 在不太偏离中心的地方, 子弹的分布是比较接近等距分布的,
...
%       \draw[help lines] (0,0) -- ({\a+90}:{sec(\a)})
%       node[dot,black] {};
%       \node[left] at (0,0) {$O$};
%     \end{tikzpicture}
%   \end{center}
