...
设向量$\vec{a}=(a_1,...,a_n),\vec{b}=(b_1,...,b_n)$.
$\vec{a}$和$\vec{b}$的加法记作$\vec{a}+\vec{b}$,
减法记作$\vec{a}-\vec{b}$, 计算方法为
\begin{align*}
  \vec{a}+\vec{b} & = (a_1+b_1,...,a_n+b_n), \\
  \vec{a}-\vec{b} & = (a_1-b_1,...,a_n-b_n). \\
\end{align*}
...
向量$\vec{a}=(a_1,...,a_n)$和数值$\lambda$的乘法称为数乘,
沿用乘法的记法, 记作$\lambda\cdot\vec{a}$,
可以省略乘号, 记作$\lambda\vec{a}$. 计算方法为
\[
  \lambda\vec{a} = (\lambda a_1,...,\lambda a_n).
\]
...
\begin{theorem}[线性插值]
  设$n$维空间中有两个不重合的点$A=(a_1,...,a_n),B=(b_1,...,b_n)$.
  对于空间中的任一点$C=(c_1,...,c_n)$,
